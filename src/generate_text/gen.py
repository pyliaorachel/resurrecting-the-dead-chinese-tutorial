...
def gen_text(model, patterns, char_to_int, int_to_char, chars, n_sent=10, restart_seq=False):
    ...
    while cnt < n_sent: 
        ...
        pred = model(seq_in).detach().view(-1)
        pred = torch.softmax(pred, dim=0)              # take softmax to turn into probability distribution
        char = np.random.choice(chars, p=pred.numpy()) # pick char based on probability, instead of always picking the highest value
